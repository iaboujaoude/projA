...
def calc_mean_BCE_from_probas(ytrue_N, yproba1_N):
    ...
    bce_vals = - (ytrue_N * np.log2(yproba1_N) + (1 - ytrue_N) * np.log2(1 - yproba1_N))
    
    bce = np.mean(bce_vals)
...
def calc_mean_BCE_from_scores(ytrue_N, scores_N):
    ...
    scores_and_zeros_N2 = np.column_stack((np.zeros((N)), flipped_scores_N))  # TODO fix me
    ...
    logsumexp_arr = scipy_logsumexp(scores_and_zeros_N2, axis=1)
    bce_score = np.mean(logsumexp_arr) / np.log(2)
