...
class LogisticRegressionGradientDescent():
    ...
    * the extra L2 penalty term will only ever add to the loss
    ...
    def predict_proba(self, x_NF):
        ...
        z_N = np.dot(x_NF, w_F) + bias  # TODO compute weights times x plus b
        ...
        proba_N2 = np.vstack([proba0_N, proba1_N]).T  # TODO returnval fulfill docstring
    ...
    def predict(self, x_NF):
        ...
        proba_N1 = proba_N2[:, 1]
        yhat_bool_N = (proba_N1 > thr)  # TODO fixme
        ...
        * trace_grad_L1_norm contains L1 norm of grad after every step
        ...
        for iter_id in range(self.num_iterations + 1):
            if iter_id > 0:
                ...
                wtil_G = wtil_G - (self.step_size * grad_G)  # <- TODO replace this line

            loss = self.calc_loss(wtil_G, xone_NG, y_N)
            grad_G = self.calc_grad(wtil_G, xone_NG, y_N)  # TODO call calc_grad
    ...
    def calc_loss(self, wtil_G, xone_NG, y_N):
        ...
        w_G_minus = wtil_G[:-1] 
        L2_loss = 0.5 * np.dot(w_G_minus, w_G_minus)

        # Second term: Calc scoreBCE summed over all examples in dataset
        labels_N = np.asarray(y_N, dtype=np.int32)

        activation_N = np.dot(xone_NG, wtil_G)
        flip_signs_N = -1 * np.sign(labels_N - 0.001)  # Keeps label mapping consistent
        activation_N = np.asarray(activation_N, dtype=np.float64)

        adjusted_scores = flip_signs_N * activation_N
        stacked_scores = np.column_stack([np.zeros(int(activation_N.shape[0])), adjusted_scores])
        computed_losses = logsumexp(stacked_scores, axis=1)
        sumBCE_loss = np.sum(computed_losses)
    ...
    def calc_grad(self, wtil_G, xone_NG, y_N):
        ...
        grad_L2_wrt_wtil_G = np.zeros(G)
        grad_L2_wrt_wtil_G[:-1] = wtil_G[:-1]

        z_score = np.dot(xone_NG, wtil_G)
        proba_N = logistic_sigmoid(z_score)

        grad_sumBCE_wrt_wtil_G = np.dot(xone_NG.T, proba_N - y_N)
