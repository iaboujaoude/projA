...
def calc_k_nearest_neighbors(data_NF, query_QF, K=1):
    ...
    '''
    data_NF = data_NF.astype(float)
    query_QF = query_QF.astype(float)
    
    distances_Q = np.linalg.norm(query_QF[:, None, :] - data_NF[None, :, :], axis=2)
    nearest_i_Q = np.argsort(distances_Q, axis=1)[:, :K]
    neighbor = data_NF[nearest_i_Q]
