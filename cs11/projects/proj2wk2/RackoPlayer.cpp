                           ...
                           bool graphics_on)
{
    ...
    //Add cards from the top of the deck to the hands
...
bool RackoPlayer::play()
{
    ...
    //Exit if the user chose q to quit
    if (choice == 'q') {
        return false;
    }
    Card drawn = get_new_card(choice);
    
    //Different print statements depending on where you drew from
    
    if (choice == 'g') {
        cout << "You drew card " << drawn.value() << 
...
{
    //If the cards are not in right progressive order return false
    for (unsigned int i = 0; i < (m_hand.num_cards() - 1); i++){
        if (not (m_hand.get_card_at(i).value() < 
                 m_hand.get_card_at(i+1).value()))
        {
            return false;
        }
    }
...
char RackoPlayer::discard_gamedeck_or_quit()
{
    ...
    // Repeat prompt while g q and d are not selected
    while ((choice != 'g') and (choice != 'q') and (choice != 'd')){
        cout << CHOICE_PROMPT;
        cin >> choice;
    }
...
Card RackoPlayer::get_new_card(char choice)
{
    ...
    if (choice == 'd') {
        drawing = m_discard_pile_p->draw_top_card();
    }
    
    if (choice == 'g') {
        drawing = m_game_deck_p->draw_top_card();
    }
...
{ 
    ...
    do {
        ...
        cout << REPLACE_CARD_PROMPT;
        value_to_drop = read_int_from_cin();
        card_found = card_is_in_hand(value_to_drop);
        
        //For putting down the new card
        if (new_card.value() == value_to_drop) {
            m_discard_pile_p->put_card_on_top(new_card);
            discarded_a_card = true;
        }
        
        //Checks to see if the card is in the hand and replaces it
        if (card_found) {
            replacing_card(new_card, value_to_drop);
            discarded_a_card = true;
        }
...
//
// Entering a name using cin and returning that name
//
string RackoPlayer::your_name(){
    string name;
    cin >> name;
    return name;
}
...
//
// Input a number and check to see if that number matches any of the cards in 
// hand
//
bool RackoPlayer::card_is_in_hand(int val_check) {
    for (unsigned int i = 0; i < m_hand.num_cards(); i++) {
        if(val_check == m_hand.get_card_at(i).value()) {
            return true;
        }
    }
    return false;
...
//
// Replaces a card from the hand
void RackoPlayer::replacing_card(Card new_card, int value_to_drop) {
    
    //Traverse the card list
    for (unsigned int i = 0; i < m_hand.num_cards(); i++) {
        
        //Find a match for the given value and the card value
        if(value_to_drop == m_hand.get_card_at(i).value()) {
            m_discard_pile_p->put_card_on_top(m_hand.get_card_at(i));
