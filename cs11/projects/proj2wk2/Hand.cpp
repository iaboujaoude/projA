...
void Hand::print()
{    
    if (m_card_list.num_cards() != 0) //Only excecute if the hand is not empty
    {
        //Travetse the hand to print each card
        for (unsigned int i = 0; i < (m_card_list.num_cards()); i++) {
            cout << get_card_at(i).value();
            if (i != (m_card_list.num_cards() - 1)) {
                cout << ", ";
            }
        }
    }
