 ...
 *    Effects: None
 */
Datum WordInstance::make_datum(int filePath, int lineNum, 
                               int lineIndex, bool first) {
    Datum new_datum;

    new_datum.filePath = filePath;
    new_datum.lineNum = lineNum;
    new_datum.lineIndex = lineIndex;
 ...
 *    Effects: None
 */
void WordInstance::print_datum(Datum datum, ostream &out, vector<string>& lines, vector<string>& paths) {
    //cout << "           in print datum\n";
    out << paths[datum.filePath] << ":" << datum.lineNum << ": "
    << lines[datum.lineIndex] << endl;
}
 ...
 *    Effects: None
 */
void WordInstance::print_list(ostream &out, vector<string>& lines, vector<string>& paths) {
    for (unsigned int i = 0; i < word_vector.size(); i ++) {
        print_datum(word_vector.at(i), out, lines, paths);
    }
}
 ...
 *    Effects: none
 */
void WordInstance::insensitive_print(ostream &out, vector<string>& lines, vector<string>& paths) {
    // cout << "       in insensitive print instance\n";
    // cout << "       word_vector size is: " << word_vector.size() << endl;
    for (unsigned int i = 0; i < word_vector.size(); i ++) {
...
bool WordInstance::datum_same_last(Datum test) {
    ...
    return (word_vector.back().filePath == test.filePath) and
           ...
           (word_vector.back().lineIndex == test.lineIndex);
}
